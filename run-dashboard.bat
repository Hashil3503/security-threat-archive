...
:: 2. Spring Boot 서버 시작
echo [2/3] Spring Boot 서버(Port: 8082) 실행 시작...
...
:RUN_NGROK
echo.
echo ngrok 터널링(Port: 8082 -> 외부 HTTPS)을 시작합니다...
echo 새로운 터미널 창에서 ngrok이 실행됩니다.
start "ngrok Tunneling (Port 8082)" cmd /k ""%NGROK_PATH%" http 8082"
echo.
echo ===================================================
echo 모든 서비스 기동 명령을 전송했습니다.
echo 1. Spring Boot 서버가 정상적으로 기동되었는지 콘솔을 확인해 주세요.
echo 2. ngrok 콘솔에서 생성된 HTTPS 외부 접속 링크(https://xxx.ngrok-free.app)를 확인해 주세요.
echo ===================================================
pause
